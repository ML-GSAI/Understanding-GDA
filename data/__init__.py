...
def get_data_info(data_dir):
    ...
    if 'cifar100' in data_dir:
        ...
        from .cifar10 import DATA_DESC #this
...
def load_data(data_dir, batch_size=512, batch_size_test=256, num_workers=4, use_augmentation='none', use_consistency=False, shuffle_train=True, 
              aux_data_filename=None, unsup_fraction=None, aux_take_amount=None, validation=False):
    """
    ...
    Arguments:
        ...
        aux_take_amout (int): number of augmentation.
    ...
    if validation:
        ...
        train_dataset, test_dataset, val_dataset = load_dataset_fn(data_dir=data_dir, use_augmentation=use_augmentation, use_consistency=use_consistency,
                                                                   aux_data_filename=aux_data_filename, aux_take_amount=aux_take_amount, validation=True)
    else:
        train_dataset, test_dataset = load_dataset_fn(data_dir=data_dir, use_augmentation=use_augmentation, aux_data_filename=aux_data_filename, aux_take_amount=aux_take_amount)
    ...
    if dataset in SEMISUP_DATASETS:
        ...
        test_dataloader = DataLoader(test_dataset, batch_size=batch_size_test, shuffle=False, num_workers=4, pin_memory=False)
        
        if validation:
            val_dataloader = DataLoader(val_dataset, batch_size=batch_size_test, shuffle=False, num_workers=4, pin_memory=False)
